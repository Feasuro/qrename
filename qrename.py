...
tr = lambda string: QApplication.translate("qrename", string)
...
class RenameWindow(QMainWindow):
    ...
    def setup_ui(self) -> None:
        ...
        tabs = QTabWidget()
        tabs.addTab(self.b_renamer, tr("Basic Renamer"))
        tabs.addTab(self.a_renamer, tr("Advanced Renamer"))
        tabs.currentChanged.connect(self.set_renamer)
        self.setCentralWidget(tabs)
        #left dock for files management
        self.old_names = QListWidget()
        open_button = QPushButton(tr("Open Files"))
        open_button.clicked.connect(self.open_files)
        add_button = QPushButton(tr("Add Files"))
    ...
    def open_files(self, add=False) -> None:
        ...
        files , _ = QFileDialog.getOpenFileNames(self, tr("Select Files"), "", tr("All Files (*)"))
        if add:
        ...
        for index, file in enumerate(self.files):
            ...
            if answer != QMessageBox.StandardButton.YesToAll and os.path.lexists(new_name):
                ...
                answer = QMessageBox.question(
                    self,
                    tr("File exists"),
                    tr("File {} already exists, do you want to overwrite it?").format(new_name),
            ...
            if answer is None or answer == QMessageBox.StandardButton.Yes or answer == QMessageBox.StandardButton.YesToAll:
               ...
               tr("FileNotFoundError: The file '{}' was not found.").format(file))
                except PermissionError:
                    QMessageBox.critical(self, tr("Error"),
               tr("PermissionError: Permission denied when renaming the file '{}'.").format(file))
                except IsADirectoryError:
                    QMessageBox.critical(self, tr("Error"),
               tr("IsADirectoryError: '{}' is a directory, not a file.").format(file))
                except OSError as e:
                    QMessageBox.critical(self, tr("Error"),
               tr("OSError: An OS error occurred when renaming '{}': {}").format(file, e))
                except Exception as e:
                    QMessageBox.critical(self, tr("Error"),
               tr("Unexpected error when renaming '{}': {}").format(file, e))
                else:
...
class ArgumentParser(QCommandLineParser):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setApplicationDescription(tr("Rename files based on user-provided parameters."))
        self.addHelpOption()
        self.addVersionOption()
        self.addPositionalArgument(tr("file"),
                                   tr("files to be renamed or containing directory path"),
                                   tr("[directory/file1 file2 ...]"))
        ...
        if self.positionalArguments():
            for arg in self.positionalArguments():
                if not os.path.lexists(arg):
                    print(tr("Error: File '{}' does not exist.\n").format(arg))
