...
tr = lambda string: QCoreApplication.translate("qrename", string)
...
class BasicRenamer(Renamer):
    ...
    def setup_layout(self) -> None:
        ...
        # Basic controls
        layout = QGridLayout()
        layout.addWidget(QLabel(tr("Prefix:")), 0, 0)
        layout.addWidget(self.ptype, 0, 1)
        layout.addWidget(self.pvalue, 0, 2)
        layout.addWidget(QLabel(tr("Suffix:")), 1, 0)
        layout.addWidget(self.stype, 1, 1)
        layout.addWidget(self.svalue, 1, 2)
        layout.addWidget(QLabel(tr("Name:")), 2, 0)
        layout.addWidget(self.ntype, 2, 1)
        layout.addWidget(self.nvalue, 2, 2)
        layout.addWidget(QLabel(tr("Extension:")), 3, 0)
        layout.addWidget(self.etype, 3, 1)
        layout.addWidget(self.evalue, 3, 2)
        # Number
        group = QGroupBox(tr("Number"))
        grouplayout = QFormLayout()
        grouplayout.addRow(tr("Start number:"), self.start)
        grouplayout.addRow(tr("Number of digits:"), self.digits)
        group.setLayout(grouplayout)
        layout.addWidget(group, 4, 0, 1, 3)
        # Date & Time
        group = QGroupBox(tr("Date and Time"))
        grouplayout = QFormLayout()
        grouplayout.addRow(tr("Date:"), self.date)
        grouplayout.addRow(tr("Date format:"), self.dformat)
        group.setLayout(grouplayout)
    ...
    def setup_controls(self) -> None:
        ...
        self.ptype.addItems([tr("Custom"), tr("Number"), tr("Date")])
        ...
        self.stype.addItems([tr("Custom"), tr("Number"), tr("Date")])
        ...
        self.ntype.addItems([tr("Custom name"), tr("Source name"),tr("lower case"),
                             tr("UPPER CASE"), tr("Title Case"), tr("Capitalize text")])
        ...
        self.etype.addItems([tr("Custom extension"), tr("Source extension"), tr("lower case"),
                             tr("UPPER CASE"), tr("Title Case"), tr("Capitalize text")])
...
class AdvancedRenamer(Renamer):
    ...
    def setup_layout(self) -> None:
        ...
        layout = QFormLayout()
        layout.addRow(tr("Pattern regular expression:"), self.inreg)
        layout.addRow(tr("Replacement:"), self.outreg)
        layout.addRow(tr("Maximal number of replacements:"), self.count)
        self.setLayout(layout)
    ...
    def transform(self, path: str, index: int) -> str:
        ...
        try:
            ...
            print(tr("Warning: Invalid regular expression: {}").format(e))
